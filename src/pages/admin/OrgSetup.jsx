...
const OrgSetup = () => {
  ...
  const validationSchema = {
    requiredFields: ["orgName", "email", "phone", "orgTypeId", "userId", "isMultiBranch"],
    orgName: (value) =>
    ...
    orgTypeId: (value) =>
      !value ? "Please select an organization type" : null,
    userId: (value) =>
  ...
  useEffect(() => {
    ...
    const fetchOrgTypes = async () => {
      try {
        const response = await fetch(`${baseUrl}/org-type/get-all-org-type`);
        if (!response.ok) {
        ...
        // Map organization types to Dropdown format
        const mapped = result.map((type) => ({
          label: type.orgTypeName,
          value: type._id, // Use _id as orgTypeId
        }));
        setOrgTypes(mapped);
